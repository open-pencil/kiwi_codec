defmodule KiwiCodec.DSL do
  ...
  defmacro __before_compile__(env) do
    ...
    props = build_props(kind, fields, enum_values)
    ...
    quote do
      @spec __kiwi_props__() :: KiwiCodec.MessageProps.t()
      def __kiwi_props__, do: unquote(Macro.escape(props))

      def __kiwi_definition__,
        do: unquote(Macro.escape(%{kind: kind, fields: props.ordered_fields}))

      unquote(define_shape(kind, struct_fields, enum_values, type_fields))
    end
  end

  defp build_props(kind, fields, enum_values) do
    field_props =
    ...
    %MessageProps{
      kind: kind,
      ordered_fields: field_props,
      fields_by_id: Map.new(field_props, &{&1.id, &1}),
      fields_by_name: Map.new(field_props, &{&1.name, &1}),
